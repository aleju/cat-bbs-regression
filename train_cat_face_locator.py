...
GRAYSCALE = False
SAVE_EXAMPLES = True
SAVE_EXAMPLES_DIR = os.path.join(CURRENT_DIR, "out_val_predictions")
SAVE_TRAINING_IMAGES = True
SAVE_TRAINING_IMAGES_DIR = os.path.join(CURRENT_DIR, "out_train")
...
def main():
    ...
    if SAVE_EXAMPLES:
        ...
        for img_idx, (rect_coords, coords) in enumerate(y_preds):
            image = np.copy(X_val[img_idx]) * 255
            image = np.rollaxis(image, 0, 3)
            if img_idx < 100:
                print("pred %d" % (img_idx,), "shape:", image.shape, "rect_coords:", rect_coords, "coords:", coords)
...
def get_examples_coords(count, start_at=0, augmentations=0):
    ...
    labels = np.zeros((target_count, 2 + 2 + 4), dtype=np.float32)
    img_idx = 0
    for fp_idx, image_filepath in enumerate(images_filepaths[start_at:start_at+count]):
        ...
        if image is None:
            ...
            label_prototype = np.zeros((2 + 2 + 4,), dtype=np.float32)
           ...
           # print("[get_examples_coords] Adding original coords:", coords_flat)
            if augmentations > 0:
                ...
                for i in range(augmentations):
                    ...
                    label_aug = np.zeros((2 + 2 + 4), dtype=np.float32)
...
def predict_on_images(model, images):
    ...
    for i, y_pred in enumerate(y_preds_model):
        ...
        y_preds.append((rect_coords, coords))
    return y_preds
